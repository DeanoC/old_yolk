...
namespace Swf {
void SwfRuntimeText::Display(	SwfPlayer* _player, 
	...
	for(int i = 0; i < text->numRecords; ++i) {
		...
		for(int j = 0; j < rec->numGlyphEntries; ++j) {
			...
			SwfRuntimeShape* shape = font->glyphShapes[glyph->index];
			...
			shape->Display(_player, _parent, &nmat, &fakeCT, _depth, _clipLayer, _morph);
