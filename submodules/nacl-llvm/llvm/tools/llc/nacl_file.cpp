...
string_vector* GetDefaultCommandLine() {
  ...
  static const char* common_args[] = { "-asm-verbose=false",
                                       "-filetype=obj",
                                       kBitcodeFilename,
                                       "-o",
                                       kObjectFilename };
...
void *run_streamed(void *arg) {
  int object_fd = reinterpret_cast<intptr_t>(arg);
  string_vector* cmd_line_vec = GetDefaultCommandLine();
  cmd_line_vec->push_back("-streaming-bitcode");
  if (DoTranslate(cmd_line_vec, 0, object_fd) != 0) {
    printerr("DoTranslate failed.\n");
